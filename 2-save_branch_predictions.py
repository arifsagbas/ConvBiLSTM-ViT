...
# === Tahminleri üret ve kaydet ===
for branch in branches:
    print(f"\n🔍 Tahminler çıkarılıyor: {branch.upper()}")
    ...
    np.save(os.path.join(result_dir, f"y_prob_{branch}.npy"), np.array(all_prob))
    np.save(os.path.join(result_dir, f"y_true_{branch}.npy"), np.array(all_true))
    print(f"✅ {branch.upper()} -> Tahminler kaydedildi.")
